...
def discover_all_ckpt_dirs(root: pathlib.Path) -> list[pathlib.Path]:
    """Immediate child dirs that contain ``CKPT_NAME`` (any name), sorted stably."""
    found: list[pathlib.Path] = []
    for p in root.iterdir():
        if p.is_dir() and (p / CKPT_NAME).is_file():
            found.append(p)

    def sort_key(p: pathlib.Path):
        m = re.match(r"^iter[_]?(\d+)$", p.name, re.I)
        if m:
...
def run_models(
    ...
    meta_csv = resolve_meta_csv_for_ensemble(run_dirs, device)
    ...
    for run_dir in run_dirs:
        ...
        if "encoder_state_dict" in ckpt:
            ...
            model, loader = build_loader_daep(run_dir, ckpt, meta, te_idx, wave, flux, mask, device)
        else:
            model, loader = build_loader_latent(ckpt, meta, te_idx, device)
...
def main(comparison_root: pathlib.Path | None = None) -> None:
    root = pathlib.Path(comparison_root or COMPARISON_ROOT).resolve()
    all_dirs = discover_all_ckpt_dirs(root)
    if not all_dirs:
    ...
    y_true, all_scores = run_models(run_dirs, device)
    n_runs = len(all_scores)
    average_kind = "macro" if IS_MACRO else "micro"

    f1_runs = [
        f1_score(y_true, np.argmax(s, axis=1), average=average_kind) for s in all_scores
    ]
    auc_runs = [
        roc_auc_score(y_true, s, multi_class="ovr", average=average_kind) for s in all_scores
    ]
    f1_m, f1_s = float(np.mean(f1_runs)), std_ddof1(np.array(f1_runs))
    auc_m, auc_s = float(np.mean(auc_runs)), std_ddof1(np.array(auc_runs))
    dup_note = (
        f"\n{n_dup_skipped} duplicate ckpt file(s) skipped (same MD5 as another folder)."
        if n_dup_skipped
        else ""
    )
    ...
    ax.set_title(
        f"ROC — {model_type}\n"
        f"{average_kind} F1 = {f1_m:.3f} ± {f1_s_str}  |  {average_kind} avg AUC = {auc_m:.3f} ± {auc_s_str}"
    )
    fig.tight_layout()
    out = root / f"ensemble_roc_ovr_{average_kind}.png"
    fig.savefig(out, dpi=160, bbox_inches="tight")
    plt.close(fig)


if __name__ == "__main__":
    arg_root = pathlib.Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else None
    main(arg_root)
